#!/bin/bash
# Parks-no-B rule (user decision): 00282 + 02483 flip B -> C in the
# canonical gate. Waits for the hybgate adoption job, then reruns the 15
# correction-active experiments for the two parks + full downstream.
set -e
cd /home/viktor/Work/synthetic_re_data_generation
PY=$PWD/synthre/bin/python
STATE=/tmp/round2_noB.state
LOG=/tmp/round2_noB.log
touch "$STATE"
while ! grep -q "HYBGATE ADOPTION FINISHED" /tmp/round2_adopt.log 2>/dev/null; do
  sleep 30
done
step() {
  local name="$1"; shift
...
for exp in M2 M2noage M2all M3 M3noage M3allconst M4 M4all M4all_noage \
  ...
  step "run_$exp" $PY scripts/round2/run_ladder.py "$exp" --parks 00282 02483
done
...
echo "$(date +%H:%M:%S) NOB RERUN FINISHED" >> "$LOG"
